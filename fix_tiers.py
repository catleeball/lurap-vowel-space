...
# def fix_typos(word_tier: TextgridTier, log_prefix: str = ''):
#     typos = {
#         'myn': 'mÿn',
#         'pjo': None,
#         'ŋ': None,
...
#         if label in typos:
#             old_label = entry.label
#             if typos[label] is not None:
#                 new_entry = Interval(start=entry.start, end=entry.end, label=typos[label])
#                 word_tier.insertEntry(new_entry, 'replace', 'silence')
#             replacement_log.append(f'{entry.start}\t{entry.end}\t{old_label}\t{typos[label]}')
#
#     replacement_log = '\n'.join(replacement_log)
#     replacement_log = 'START\tEND\tOLD_LABEL\tNEW_LABEL' + replacement_log
#     with open(f'{log_prefix}_word_tier_typo_log.tsv', 'w') as f:
